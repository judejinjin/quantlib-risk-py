        ...
        for (Size i = 0; i < callabilityTimes_.size(); ++i) {
            ...
            for (Size j = 0; j < couponTimes_.size(); j++) {
                ...
                if (withinNextWeek(callabilityTime, couponTime) && callabilityDate < couponDate) {
                    ...
                    // Explicit return type prevents XAD expression-template
                    // dangling references: auto would deduce a lightweight
                    // expression that holds ADVar refs to the lambda's locals
                    // (time, zeroRateInclSpread).  After the lambda returns
                    // those refs dangle, corrupting the tape on the next
                    // computeAdjoints().  Returning Real forces evaluation
                    // inside the lambda while locals are still alive.
