#!/usr/bin/env python3
"""
IR Cap — Second-Order Sensitivities via FD-over-AAD
=====================================================

Computes the full 18×18 Hessian matrix of a 10-year interest rate cap
with respect to 17 bootstrapping inputs + 1 Black volatility.

  • 1 deposit, 3 FRAs, 8 futures, 5 swap rates (yield curve)
  • 1 Black flat volatility
...
-----
  ...
  python benchmarks/second_order_ir_cap.py --no-save

Internal worker mode (invoked automatically by the orchestrator):
  python benchmarks/second_order_ir_cap.py --worker REPEATS
"""
...
_INPUT_VALS = [_DEPO_RATE] + _FRA_RATES + _FUTURES_PRICES + _SWAP_RATES + [_BLACK_VOL]
_N = len(_INPUT_VALS)

H_HESS = 1e-5  # bump size for Hessian FD

# Block indices
_RATE_IDX = list(range(17))
_VOL_IDX  = [17]
...
# ============================================================================
# Curve builder (shared between AAD and FD)
# ============================================================================

def _build_cap(input_reals):
    """Build PiecewiseFlatForward curve + 10Y cap from ql.Real objects.
    ...
    curve = ql.PiecewiseFlatForward(settlement, helpers, dc)
    curve.enableExtrapolation()

    yieldHandle = ql.YieldTermStructureHandle(curve)
    index = ql.Euribor3M(yieldHandle)

    vol_handle = ql.OptionletVolatilityStructureHandle(
        ql.ConstantOptionletVolatility(
            todaysDate, ql.TARGET(), ql.ModifiedFollowing,
            ql.QuoteHandle(ql.SimpleQuote(vol_real)),
            ql.Actual365Fixed()))

    cap_start = ql.TARGET().advance(settlement, ql.Period(2, ql.Months))
    cap_maturity = ql.TARGET().advance(cap_start, ql.Period(10, ql.Years))
    cap_schedule = ql.Schedule(
        cap_start, cap_maturity, ql.Period(ql.Quarterly), ql.TARGET(),
        ql.ModifiedFollowing, ql.ModifiedFollowing,
        ql.DateGeneration.Forward, False)

    cap = ql.Cap(ql.IborLeg([1_000_000.0], cap_schedule, index), [0.05])
    cap.setPricingEngine(ql.BlackCapFloorEngine(yieldHandle, vol_handle))

    return cap


def _compute_aad_gradient(input_vals):
    """Record tape, compute NPV + gradient for all 18 inputs."""
    import QuantLib_Risks as ql
...
def _compute_fd_npv(input_vals):
    """Build cap, return float NPV."""
    import QuantLib_Risks as ql
    import xad

    input_reals = [ql.Real(v) for v in input_vals]
    cap = _build_cap(input_reals)
    return float(xad.value(cap.NPV()))
...
def _run_worker(repeats: int) -> dict:
    ...
    blocks = [
        _block_summary(hess_aad, _RATE_IDX, _RATE_IDX, "Rate", "Rate"),
        _block_summary(hess_aad, _RATE_IDX, _VOL_IDX, "Rate", "Vol"),
        _block_summary(hess_aad, _VOL_IDX, _VOL_IDX, "Vol", "Vol"),
    ]
...
def print_results(r: dict):
    print()
    print(SEPARATOR)
    print("IR Cap — Second-Order Sensitivities (FD-over-AAD)")
    print(SEPARATOR)
    print(f"  10Y Cap on Euribor3M, strike = 5%, notional = 1M")
    print(f"  Curve: 1 depo + 3 FRAs + 8 futures + 5 swaps + Black vol = {_N} inputs")
...
MD_PATH = Path(__file__).resolve().parent / "second_order_ir_cap_results.md"
...
def write_markdown(r: dict, wheels: dict):
    ...
    w("# IR Cap — Second-Order Sensitivities (FD-over-AAD)")
    w("")
    ...
    w("## Instrument")
    w("")
    w("- 10-year Cap on Euribor3M, strike = 5%, notional = 1,000,000")
    w("- `PiecewiseFlatForward` yield curve + `BlackCapFloorEngine`")
    w(f"- **{_N} inputs:** 17 rate curve inputs + 1 Black flat volatility ({_BLACK_VOL:.0%})")
    ...
    w("")
    w("<details>")
    w("<summary>Click to expand 18×18 Hessian matrix</summary>")
    w("")
    w("| | " + " | ".join(_INPUT_NAMES) + " |")
    w("|---|" + "---:|" * _N)
    for i in range(_N):
    ...
    w("> **Key insight:** The Rate×Vol cross-gamma block captures vanna-like")
    w("> interactions between interest rate curve inputs and Black volatility.")
    w("")
    ...
    w("|---|---|---:|")
    for _, i, j, v in entries[:10]:
        w(f"| {_INPUT_NAMES[i]} | {_INPUT_NAMES[j]} | {v:.4f} |")
    w("")
    ...
    w("python benchmarks/second_order_ir_cap.py")
    w("python benchmarks/second_order_ir_cap.py --repeats 50")
    w("```")
    w("")
...
def main():
    parser = argparse.ArgumentParser(
        description="IR Cap — 2nd-order sensitivities via FD-over-AAD",
