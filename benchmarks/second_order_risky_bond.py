#!/usr/bin/env python3
"""
Risky Bond — Second-Order Sensitivities via FD-over-AAD
========================================================

Computes the full 14×14 Hessian matrix of a risky fixed-rate bond
with respect to all bootstrapped OIS and CDS inputs + recovery rate.

  • 9 OIS rates (bootstrapping `PiecewiseLogLinearDiscount`)
  • 4 CDS spreads (bootstrapping `PiecewiseFlatHazardRate`)
  • 1 recovery rate
...
-----
  ...
  python benchmarks/second_order_risky_bond.py --no-save

Internal worker mode (invoked automatically by the orchestrator):
  python benchmarks/second_order_risky_bond.py --worker REPEATS
"""
...
# Market data
# ---------------------------------------------------------------------------
_OIS_TENORS = ["1Y", "2Y", "3Y", "4Y", "5Y", "7Y", "10Y", "15Y", "20Y"]
_OIS_RATES  = [0.0483, 0.0455, 0.0425, 0.0357, 0.0340, 0.0335, 0.0350, 0.0385, 0.0415]

_CDS_TENORS  = ["1Y", "3Y", "5Y", "7Y"]
_CDS_SPREADS = [0.0050, 0.0075, 0.0100, 0.0125]

_RECOVERY = 0.40

_INPUT_NAMES = (
    [f"OIS {t}" for t in _OIS_TENORS]
    + [f"CDS {t}" for t in _CDS_TENORS]
    + ["Recovery"]
)
_INPUT_VALS = _OIS_RATES + _CDS_SPREADS + [_RECOVERY]
_N = len(_INPUT_VALS)

H_HESS = 1e-5  # bump size for Hessian FD

# Block boundaries for markdown analysis
_OIS_IDX = list(range(9))
_CDS_IDX = list(range(9, 13))
_REC_IDX = [13]
...
def _build_bond(input_reals):
    """Build risky bond from ql.Real objects.
    MUST be called after tape.newRecording() for AAD to work.
    Returns bond."""
    import QuantLib_Risks as ql

    evalDate = ql.Date(15, ql.November, 2024)
    ql.Settings.instance().evaluationDate = evalDate
    calendar = ql.TARGET()

    # -- OIS curve --
    ois_rates_real = input_reals[:9]
    ois_helpers = []
    for i, (tenor_str, rate) in enumerate(zip(_OIS_TENORS, ois_rates_real)):
        years = int(tenor_str.replace("Y", ""))
        ois_helpers.append(ql.OISRateHelper(
            2, ql.Period(years, ql.Years),
            ql.QuoteHandle(ql.SimpleQuote(rate)),
            ql.Eonia()))

    ois_curve = ql.PiecewiseLogLinearDiscount(evalDate, ois_helpers, ql.Actual365Fixed())
    ois_curve.enableExtrapolation()

    # -- CDS → hazard rate curve --
    cds_spreads_real = input_reals[9:13]
    rec_real = input_reals[13]

    ois_handle = ql.YieldTermStructureHandle(ois_curve)

    cds_helpers = []
    for i, (tenor_str, spread) in enumerate(zip(_CDS_TENORS, cds_spreads_real)):
        years = int(tenor_str.replace("Y", ""))
        cds_helpers.append(ql.SpreadCdsHelper(
            ql.QuoteHandle(ql.SimpleQuote(spread)),
            ql.Period(years, ql.Years),
            0, calendar, ql.Quarterly, ql.Following,
            ql.DateGeneration.TwentiethIMM,
            ql.Actual365Fixed(), rec_real, ois_handle))
    ...
    # -- Bond --
    issue = ql.Date(15, ql.November, 2024)
    ...
    engine = ql.RiskyBondEngine(default_handle, rec_real, ois_handle)
...
def _compute_fd_npv(input_vals):
    """Build bond, return float NPV."""
    import QuantLib_Risks as ql
    import xad

    input_reals = [ql.Real(v) for v in input_vals]
    bond = _build_bond(input_reals)
    return float(xad.value(bond.NPV()))
...
def _block_summary(hess, row_idx, col_idx, row_label, col_label):
    """Return (label, max_abs, example_i, example_j, value)."""
    max_v = 0
    best = (0, 0, 0.0)
    for i in row_idx:
        for j in col_idx:
            if abs(hess[i][j]) > max_v:
                max_v = abs(hess[i][j])
                best = (i, j, hess[i][j])
    return {
        "block": f"{row_label} × {col_label}",
        "max_abs": max_v,
        "example": f"{_INPUT_NAMES[best[0]]} × {_INPUT_NAMES[best[1]]}",
        "value": best[2],
    }
...
def _run_worker(repeats: int) -> dict:
    ...
    # Block analysis
    blocks = [
        _block_summary(hess_aad, _OIS_IDX, _OIS_IDX, "IR", "IR"),
        _block_summary(hess_aad, _CDS_IDX, _CDS_IDX, "CR", "CR"),
        _block_summary(hess_aad, _OIS_IDX, _CDS_IDX, "IR", "CR"),
        _block_summary(hess_aad, _OIS_IDX, _REC_IDX, "IR", "Rec"),
        _block_summary(hess_aad, _CDS_IDX, _REC_IDX, "CR", "Rec"),
        _block_summary(hess_aad, _REC_IDX, _REC_IDX, "Rec", "Rec"),
    ]
...
def print_results(r: dict):
    print()
    print(SEPARATOR)
    print("Risky Bond — Second-Order Sensitivities (FD-over-AAD)")
    print(SEPARATOR)
    print(f"  5Y fixed-rate bond, 5% coupon, semiannual, 100 face")
    print(f"  OIS discount + CDS-implied hazard rates + recovery")
    ...
    print("  Hessian block analysis:")
    print(f"    {'Block':>20s}  {'Max |entry|':>14s}  {'Example':>30s}  {'Value':>14s}")
    for b in r["blocks"]:
        print(f"    {b['block']:>20s}  {b['max_abs']:>14.4f}  "
              f"{b['example']:>30s}  {b['value']:>14.4f}")
    print()
...
MD_PATH = Path(__file__).resolve().parent / "second_order_risky_bond_results.md"
...
def write_markdown(r: dict, wheels: dict):
    ...
    w("# Risky Bond — Second-Order Sensitivities (FD-over-AAD)")
    w("")
    ...
    w("## Instrument")
    w("")
    w("- 5-year fixed-rate bond, 5% coupon, semiannual, 100 face value")
    w("- `PiecewiseLogLinearDiscount` OIS curve + `PiecewiseFlatHazardRate` + `RiskyBondEngine`")
    w(f"- **{_N} inputs:** 9 OIS rates + 4 CDS spreads + 1 recovery rate")
    w(f"- Hessian bump size h = {H_HESS}")
    w("- Evaluation date: 15-Nov-2024")
    w("")
    w("---")
    w("")
    ...
    w("### Hessian Block Analysis")
    w("")
    w("| Block | Max \\|entry\\| | Dominant pair | Value |")
    w("|---|---:|---|---:|")
    for b in r["blocks"]:
        w(f"| {b['block']} | {b['max_abs']:.4f} | {b['example']} | {b['value']:.4f} |")
    w("")
    ...
    w("")
    w("<details>")
    w("<summary>Click to expand 14×14 Hessian matrix</summary>")
    w("")
    w("| | " + " | ".join(_INPUT_NAMES) + " |")
    w("|---|" + "---:|" * _N)
    for i in range(_N):
    ...
    w("> **Key insight:** The IR×CR cross-gamma block shows significant")
    w("> second-order interaction between interest rate and credit risk,")
    w("> demonstrating non-trivial curvature that is missed by first-order sensitivities.")
    w("")
    ...
    w("python benchmarks/second_order_risky_bond.py")
    w("python benchmarks/second_order_risky_bond.py --repeats 50")
    w("```")
    w("")
...
def main():
    parser = argparse.ArgumentParser(
        description="Risky Bond — 2nd-order sensitivities via FD-over-AAD",
